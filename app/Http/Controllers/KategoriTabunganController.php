...
class KategoriTabunganController extends Controller
{
    public function index(Request $request)
    {
        ...
        if (request()->ajax()) {
            $result = KategoriTabungan::orderBy('kode', 'ASC');
            return DataTables::eloquent($result)
                ->addIndexColumn()
                ->addColumn('act', function ($data) {
                    return view('pages.kategoritabungan.modal.action', compact('data'));
                })
                ->rawColumns(['act'])->make(true);
        }
        return view('pages.kategoritabungan.index');
    }
    ...
    public function store(Request $request)
    {
        ...
        DB::beginTransaction();
        try {
            $kategori_tabungans = KategoriTabungan::create([
                'kode'          => $request->kode,
                'nama'   => $request->nama
            ]);
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $kategori_tabungans = KategoriTabungan::find($id)->update($data);
    ...
    public function destroy($id)
    {
        DB::beginTransaction();
        try {
            $kategori_tabungans = KategoriTabungan::find($id)->delete();
